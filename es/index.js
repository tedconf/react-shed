...
var sizes = function sizes() {
  ...
  return Array.from([].concat(_toConsumableArray(Array(number)))).reduce(function (acc, curr, i) {
    var _extends2;

    return _extends({}, acc, (_extends2 = {}, _defineProperty(_extends2, 'z' + i, ms(i - 1, scale) + 'rem'), _defineProperty(_extends2, 'z0', 0), _defineProperty(_extends2, 'z.' + (number - i), ms(i - number, scale) + 'rem'), _defineProperty(_extends2, 'z-' + i, '-' + ms(i - 1, scale) + 'rem'), _defineProperty(_extends2, 'z0', 0), _defineProperty(_extends2, 'z.-' + (number - i), '-' + ms(i - number, scale) + 'rem'), _extends2));
  }, {});
};
...
var createTheme = function createTheme() {
  ...
  if (userTheme.sizes && _typeof(userTheme.sizes) === 'object') {
    var sizesHash = Object.keys(userTheme.sizes).reduce(function (acc, curr, i) {
      return _extends({}, acc, _defineProperty({}, 'z' + curr, userTheme.sizes[curr]));
    }, {});
    if (!sizesHash['z0']) {
      sizesHash['z0'] = 0;
    }
...
var Shed = function Shed(_ref) {
  ...
  var ShedStyled = styled(component).withConfig({
